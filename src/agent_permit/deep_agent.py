...
Never write literal citation templates such as [rule:<rule_id>]. If no scanner rule IDs
exist, say no rule citations apply without using bracketed placeholder syntax.
...
def build_investigation_prompt(context: EvidenceContext) -> str:
    ...
    return "\n".join(
        [
            ...
            "Do not write literal citation templates such as [rule:<rule_id>].",
            "If no scanner rule IDs exist, say no rule citations apply without bracketed placeholders.",
